...
namespace BST {
    ...
    TreeNode::TreeNode(int _NodeOrderID, int _NodeChronolgicalID, std::string _Name, int _Age, double _Income, int _PostCode) {
    ...
    void TreeNode::print() {
        std::cout << " | " << this->getNodeChronologicalID() << " \t\t\t| " << this->getName() << " \t| " << this->getAge() << " \t| " << this->getIncome() << " \t| " << this->getPostCode() << "\t\t| " << this->getNodeOrderID() << " \t| " << std::endl;
    }
    ...
    void addToTree(TreeNode* Node, TreeNode* newNode) {
        if (Node->getNodeOrderID() > newNode->getNodeOrderID())
            if (isEnd(Node->getLeft())) Node->setLeft(newNode);
            else addToTree(Node->getLeft(), newNode);
        else
            if (isEnd(Node->getRight())) Node->setRight(newNode);
            else addToTree(Node->getRight(), newNode);
        return;
    }
